...
- Swift and Rust communicate over a Unix socket using the JSON protocol defined in `crates/macos-file-provider`. The containing app and extension share app-group entitlements, and the socket file uses the shared app-group container at `~/Library/Group Containers/<team>.group.com.hybridcipher.macOS/<root>.sock`. The filename is shortened to stay under macOS `SUN_LEN`; `/tmp/hc-fp/<root>.sock` is only a fallback when the app-group path is unavailable or too long.
